...
class Ansys():
    def __init__(self):
        self.name = ''
        self.x_axis = 'Magnetic Field (Oe)'
        self.y_axis = 'Bridge 3 Resistance (Ohms)'
        self.tick = []
        self.tick_label =[]
        self.sep = 0
        self.xlim = 0
    ...
    def read(self):  ##读取文件
        df = pd.read_csv('./REFORMATTED_BAND-soc.dat',sep='\s+',header=None)  
        # df = pd.read_csv('REFORMATTED_BAND_224.dat',sep='\s+',header=None) 
        x_list = list(df[0])
        self.xlim = x_list[-1]
    ...
    def plot(self,df):  #画图
        ...
        plt.savefig('REFORMATTED_BAND-soc.png',dpi=600)
        plt.clf()
